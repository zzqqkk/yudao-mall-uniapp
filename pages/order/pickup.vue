<template>
	...
	<view class="bg-white address-box ss-m-b-14 ss-r-b-10" @tap="onSelectAddress">
		<s-store-address-item :item="state.addressInfo" :hasBorderBottom="false" :isConfirm="true">
			<view class="ss-rest-button">
				<text class="_icon-forward item-icon" />
			</view>
		</s-store-address-item>
	</view>
	...
	<view class="order-card-box ss-m-b-14">
		<s-goods-item v-for="item in state.orderInfo.items" :key="item.skuId" :img="item.picUrl" :title="item.spuName"
			...
			:num="item.count" marginBottom="10" />
		<view class="order-item ss-flex ss-col-center ss-row-between ss-p-x-20 bg-white ss-r-10">
			<view class="item-title">联系人</view>
			<view class="ss-flex ss-col-center">
				<uni-easyinput maxlength="20" placeholder="请填写联系人昵称" v-model="state.orderPayload.receiverName"
		...
		<view class="order-item ss-flex ss-col-center ss-row-between ss-p-x-20 bg-white ss-r-10">
			...
			<view class="ss-flex ss-col-center">
				<uni-easyinput maxlength="20" placeholder="请填写联系人手机号码" v-model="state.orderPayload.receiverMobile"
					:inputBorder="false" :clearable="false" />
			</view>
		</view>
		<view class="order-item ss-flex ss-col-center ss-row-between ss-p-x-20 bg-white ss-r-10">
		...
		<view class="total-box-content border-bottom">
			...
			<view class="order-item ss-flex ss-col-center ss-row-between" v-if="state.orderInfo.type === 0">
				...
				<view class="ss-flex ss-col-center" @tap="state.showCoupon = true">
					...
					<text class="item-value" :class="state.couponInfo.length > 0 ? 'text-red' : 'text-disabled'" v-else>
						{{
...
<script setup>
	...
	function onSelectAddress() {
		...
		sheep.$router.go('/pages/user/address/store-list');
	}
	...
	async function submitOrder() {
		if(!state.orderPayload.couponId > 0){
			state.orderPayload.couponId = undefined;
		}
		const {
			code,
			data
			...
			pickUpStoreId: state.addressInfo.id,
			receiverName: state.orderPayload.receiverName,
			receiverMobile: state.orderPayload.receiverMobile,
			deliveryType: 2, // 门店自提
